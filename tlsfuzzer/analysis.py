...
class Analysis(object):
    ...
    def _k_sizes_totals(self, name_bin):
        ...
        with mp.Pool(self.workers) as p:
            chunks = p.imap_unordered(
                ...
                ((k_sizes, i) for i in _slices(total_data, 1024*1024*64))
            )
    ...
    def _bit_size_smart_analysis_worker(self, args):
        tuple_ids, k_sizes, values, bounds = args
        start, end = bounds
        max_k_size_value = -1
        prev_tupple_id = -1
        tuple_id = 0
    ...
    def _figure_out_analysis_data_size(self):
        ...
        tuple_nums = data['tuple_num']
        k_size_list = data['k_size']
        value = data['value']
        ...
        with mp.Pool(self.workers) as p:
            chunks = p.imap_unordered(
                ...
                ((tuple_nums, k_size_list, value, i) for i in
                 _slices(total_data, 1024*1024*64))
            )

            chosen_tuples = []
            for subprocess_tuples in chunks:
