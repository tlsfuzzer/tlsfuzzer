...
class Analysis:
    ...
    def _convert_to_binary(self):
        ...
        # load 51200 rows at a time so that we don't use more than 200MiB
        # (including pandas overhead) of memory at a time to process a file with
        # 256 columns
        # XXX: increase once we confirm we can process multi-gigabyte
        # files with just few hundred megs of memory
        csv_reader = pd.read_csv(timing_csv_path, chunksize=51200,
                                 dtype=np.float64)
