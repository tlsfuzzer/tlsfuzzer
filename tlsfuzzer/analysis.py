...
class Analysis(object):
    ...
    def _bit_size_come_to_verdict(self, analysis_ret_val):
        ...
        if analysis_ret_val != 0:
            explanation = ("Definite side-channel detected, "
                           "implementation is VULNERABLE.")
        elif self.skillings_mack_pvalue < 1e-9:
            explanation = ("Definite side-channel detected, "
                           "implementation is VULNERABLE.")
        elif self.skillings_mack_pvalue < 1e-5:
            ...
            for k_size in top_k_sizes:
                ...
                for method in bootstraping_of_size:
                    if larger_ci < bootstraping_of_size[method][1]:
                        larger_ci = bootstraping_of_size[method][1]
            ...
            if larger_ci < 1e-10:
                ...
                explanation = ("Large confidence intervals detected, "
                               "collecting more data necessary. Side channel "
                               "leakage smaller than {0:.3e}s is possible."
                               .format(larger_ci))
            else:
    ...
    def _bit_size_write_summary(self, verdict):
        ...
        with open(join(self.output, "report.txt"), "w") as fp:
            fp.write(
                "Skilling-Mack test p-value: {0:.2e}\n"
                    .format(self.skillings_mack_pvalue) +
    ...
    def generate_report(self, bit_size=False):
        ...
        """
        if bit_size:
            self.skillings_mack_pvalue = self.skillings_mack_test()
            ret_val = self.analyze_bit_sizes()
            difference, verdict = self._bit_size_come_to_verdict(ret_val)
            self._bit_size_write_summary(verdict)
