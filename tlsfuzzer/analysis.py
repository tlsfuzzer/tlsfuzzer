...
class Analysis(object):
    ...
    def conf_interval_plot(self):
        ...
        for name, data in boots.items():
            with open(join(self.output,
                    "bootstrapped_{0}.csv".format(name)), "w") as f:
                writer = csv.writer(f)
            ...
            ax.set_title("Confidence intervals for {0} of differences".format(name))
            ...
            canvas.print_figure(join(self.output,
                                     "conf_interval_plot_{0}.png".format(name)),
                                bbox_inches="tight")
    ...
    def _write_summary(self, difference, p_vals, sign_p_vals, worst_pair,
        ...
        with open(text_report_filename, 'w') as txt_file:
            ...
            self._write_stats("Median", low_median, median, high_median,
                txt_file)
            self._write_stats(
            ...
            self._write_stats("Trimean", low_trimean, trimean, high_trimean,
                txt_file)
