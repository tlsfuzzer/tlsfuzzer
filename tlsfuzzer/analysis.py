...
def help_msg():
 ...
 --no-smart-analysis By default the script will compute how much data are
                   needed to calculate small confidence interval to the 4th
                   bit size and use only this number of data (if available).
                   This option disables this feature and uses all the available
                   data.
 --bit-size-desire-ci num The desired amount of ns (or lower) that the CIs
...
class Analysis(object):
            ...
            try:
                ...
                if self.verbose:
                    print("[i] Skillings-Mack test done in {:.3}s".format(
                        time.time() - start_time))
                    print("[i] Skillings-Mack pvalue: {0:.6e}".format(
                        sm_test.p_value))
                    status[2].set()
                    progress.join()
                    print()
    ...
    def _figure_out_analysis_data_size(self, k_sizes):
        ...
        if self.bit_size_recall_size > len(k_sizes):
            self.bit_size_recall_size = 4 if len(k_sizes) > 4 else len(k_sizes)
        ...
        smaller_recall_ci = min(filter(lambda x: x > 0, all_recall_cis))
        magnitude_diff = np.ceil(smaller_recall_ci / self.bit_size_desire_ci)
        self._bit_size_data_limit = int(
            np.square(magnitude_diff) * self._bit_size_data_used
        )
