...
"""Test of ML-DSA in CertificateVerify"""
...
def main():
    ...
    cr_sigalgs = [SignatureScheme.mldsa87,
    ...
    if private_key.key_type == "mldsa87":
        sig_alg = SignatureScheme.mldsa87
        msg_alg = SignatureScheme.mldsa65
    elif private_key.key_type == "mldsa65":
        sig_alg = SignatureScheme.mldsa65
        msg_alg = SignatureScheme.mldsa44
    else:
        assert private_key.key_type == "mldsa44"
        sig_alg = SignatureScheme.mldsa44
        msg_alg = SignatureScheme.mldsa65
    ...
    for pos in range(siglen):
        for xor in [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80]:
            ...
            ext[ExtensionType.signature_algorithms_cert] = \
                SignatureAlgorithmsCertExtension().create(
                    MLDSA_SIG_ALL + ECDSA_SIG_ALL + RSA_SIG_ALL +
                    EDDSA_SIG_ALL)
    ...
    for name, subs in [
        ("All-zero c-tilde value in signature is rejected",
         dict((i, 0) for i in range(0, c_tilde_len))),
        ("All-zero z value in signature is rejected",
         dict((i, 0) for i in range(c_tilde_len, c_tilde_len + z_len))),
        ("All-zero h value in signature is rejected",
         dict((i, 0) for i in range(c_tilde_len + z_len,
                                    c_tilde_len + z_len + h_len))),
    ...
    print("ML-DSA signatures in TLS1.3")
    print("Test should be executed three times, once each with ML-DSA-44,")
    print("ML-DSA-65 and ML-DSA-87 (if all are supported by the server).\n")
