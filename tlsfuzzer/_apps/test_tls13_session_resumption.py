...
def help_msg():
    ...
    print(" --skip-1.2     Skip tests that target TLS 1.2")
    ...
    print("                respected. Available PSK_ONLY and PSK_WITH_DHE.")
    print("                Default PSK_WITH_DHE,PSK_ONLY.")
    print(" --server-preferred-pke The preferred PKE of the server. If not ")
    print("                specified it will get the first from the client")
    print("                PKE list. Available PSK_ONLY and PSK_WITH_DHE.")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:x:X:n:d", ["help", "skip-1.2",
                                                       "client-pke=",
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            unknown_pke = [x for x in pke_args
                           if x.upper() not in ("PSK_WITH_DHE", "PSK_ONLY")]
            if unknown_pke:
                raise ValueError(
                    "Unknown PKE argument {0}. ".format(unknown_pke[0]) +
                    "Please use PSK_WITH_DHE or PSK_ONLY"
                )

            for pke_arg in pke_args:
                pke_arg = pke_arg.upper()

                if (pke_arg == "PSK_WITH_DHE" and
                        PskKeyExchangeMode.psk_dhe_ke not in client_pke_list):
                    client_pke_list.append(PskKeyExchangeMode.psk_dhe_ke)
                elif (pke_arg == "PSK_WITH_DHE" and
                        PskKeyExchangeMode.psk_ke not in client_pke_list):
                    client_pke_list.append(PskKeyExchangeMode.psk_ke)
        elif opt == '--server-preferred-pke':
            if arg.upper() == "PSK_WITH_DHE":
                server_preferred_pke = PskKeyExchangeMode.psk_dhe_ke
            elif arg.upper() == "PSK_ONLY":
                server_preferred_pke = PskKeyExchangeMode.psk_ke
            else:
                raise ValueError(
    ...
    if len(client_pke_list) > 1:
        for pke in client_pke_list:
            ...
            ext[ExtensionType.psk_key_exchange_modes] = \
                PskKeyExchangeModesExtension().create([pke])
            ...
            if pke == PskKeyExchangeMode.psk_ke:
                pke_name = "PSK_ONLY"
            conversation_name = "session resumption - {}".format(pke_name)
