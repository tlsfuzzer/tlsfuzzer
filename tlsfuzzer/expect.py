...
class ExpectServerHello(ExpectHandshake):
    ...
    def _check_downgrade_protection(self, srv_hello):
        ...
        elif self.srv_max_prot == (3, 3):
            if srv_hello.server_version == (3, 3):
                if srv_hello.random[24:] == TLS_1_2_DOWNGRADE_SENTINEL:
                    raise AssertionError(
                        "Server set downgrade protection sentinel but "
                        "shouldn't have done that")
            elif srv_hello.random[24:] != TLS_1_1_DOWNGRADE_SENTINEL:
                raise AssertionError(
                    "Server failed to set downgrade protection sentinel in "
                    "ServerHello.random value")
        elif self.srv_max_prot != None and srv_hello.server_version < (3, 2):
