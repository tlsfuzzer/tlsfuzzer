...
class CertificateVerifyGenerator(HandshakeProtocolMessageGenerator):
    ...
    def _select_sig_alg(self, accept_sig_algs):
        """
        Select acceptable signature algorithm based on private key and
        the CertificateRequest from server.
        """
        for sig in accept_sig_algs:
            if self.private_key.key_type == "rsa-pss":
                if sig in RSA_PSS_PSS_ALL:
                    return sig
            elif self.private_key.key_type == "ecdsa"\
                    and self.sig_version < (3, 4):
                if sig in ECDSA_SIG_ALL:
                    return sig
            elif self.private_key.key_type == "ecdsa" and \
                    self.sig_version > (3, 3):
                h_alg = curve_name_to_hash_tls13(self.private_key.curve_name)
                # the tls1.3 ecdsa SignatureSchemes reused assigned code points
                # so we just need to know the hash to create valid
                # SignatureScheme
                if sig == (getattr(HashAlgorithm, h_alg),
                           SignatureAlgorithm.ecdsa):
                    return sig
            else:
                assert self.private_key.key_type == "rsa"
                if sig in RSA_PSS_RSAE_ALL:
                    return sig
                # as a fallback check for pkcs1 only if TLS < 1.3
                if (self.sig_version < (3, 4) and
                        sig in RSA_PKCS1_ALL):
                    return sig
        # if none match, select something that matches our key
        if self.private_key.key_type == "rsa":
            return RSA_PSS_RSAE_ALL[0]
        if self.private_key.key_type == "rsa-pss":
            return RSA_PSS_PSS_ALL[0]
        assert self.private_key.key_type == "ecdsa"
        if self.sig_version == (3, 3):
            return ECDSA_SIG_ALL[0]
        h_alg = curve_name_to_hash_tls13(self.private_key.curve_name)
        return (getattr(HashAlgorithm, h_alg), SignatureAlgorithm.ecdsa)

    @staticmethod
    def _sig_alg_for_certificate(cert, accept_sig_algs, version):
        """
        Select an acceptable signature algorithm based on certificate and
        protocol version.
        """
        if cert.certAlg == "rsa":
        ...
        hash_name = curve_name_to_hash_tls13(cert.publicKey.curve_name)
            ...
            if cert_req is not None:
                if not self.private_key and not our_cert:
                    ...
                    self.msg_alg = self._sig_alg_for_certificate(
                        our_cert, cert_req.supported_signature_algs,
                        self.sig_version)
                else:
                    self.msg_alg = self._select_sig_alg(
                ...
                self.msg_alg = self._sig_alg_for_certificate(
                    our_cert, SIG_ALL, self.sig_version)
