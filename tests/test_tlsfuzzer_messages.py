...
class TestHeartbeatGenerator(unittest.TestCase):
    ...
    def test_generate_with_small_padding(self):
        hbg = HeartbeatGenerator(bytearray(b''), padding=bytearray(b'\x00'))
    ...
    def test___init___with_mutualy_exclusive_params(self):
        with self.assertRaises(ValueError) as e:
            HeartbeatGenerator(bytearray(b''), padding=bytearray(b'\x00'),
                    padding_length=1)

        self.assertIn("either padding or padding_length", str(e.exception))
