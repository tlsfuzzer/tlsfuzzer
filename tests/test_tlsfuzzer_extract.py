...
class TestMeasurementCreation(unittest.TestCase):
    def setUp(self):
        self.builtin_open = open
        self.print_looking_value = None
        self.print_found_value = False

    def custom_generator(self, data):
        for item in data:
            yield item

    def print_emulator(self, *args, **kwargs):
        if self.print_looking_value \
                and self.print_looking_value in args:
            self.print_found_value = True
    ...
    def test_measurement_creation_with_hamming_weigt(
        ...
        extract.process_measurements_and_create_csv_file(
            extract.ecdsa_iter(return_type="hamming-weight"),
    ...
    def test_measurement_creation_with_incomplete_times(
        ...
        mock_file.side_effect = self.file_emulator
        mock_print.side_effect = self.print_emulator
        self.print_looking_value = "Left over times in measurements"
        self.print_found_value = False
        ...
        self.assertTrue(self.print_found_value,
                        "There should be time left overs")
