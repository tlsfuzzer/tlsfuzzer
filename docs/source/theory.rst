...
By modifying the messages sent, tester can check if the other side can
establish the connection in expected circumstances or aborts the connection if
the other side violates the protocol or attempts to establish a connection with
unimplemented or turned off features. Same for the integrity, authentication,
and encryption, by modifying the data sent to the other side from the values
specified by the protocol, tester can verify if the other side
implements the checks that provide those properties.
...
The record layer can combine provided data or messages to it into a single
...
encryption of data.
It uses the keys and ciphers negotiated by the
...
on transmission of specific fragmentation of writes to the other side.
...
use this kind of approach, testing of negative test cases in
security protocols must use malformed or unexpected messages.
Correct handling of unexpected situations provides the security in the
protocol.
