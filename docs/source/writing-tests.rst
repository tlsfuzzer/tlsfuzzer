...
errors).
Example below builds a single tree and executes it.
...
connection.
:py:class:`~tlsfuzzer.messages.Connect` takes server's hostname and a port
number to do that:
...
Next step requires sending the first message of :term:`TLS` handshake:
the ClientHello.
...
To establish connection with ones that use :term:`ECDHE` key exchange and
...
Connections that use :term:`ECDHE` key exchange need to advertise to server
the elliptic curves supported by client.
...
To send that list to server, package it into a :term:`TLS` extension object.
...
Since servers sign :term:`ECDHE` key exchange, client needs to advertise
the signature algorithms it supports.
...
Since ServerKeyExchange message includes the group selected by server,
...
and Finished, script needs thus to expect them:
...
What happens after handshake depends on application protocol that uses
:term:`TLS`.
...
``close_notify`` while others don't, closing the connection instantly,
...
.. tip::
    ...
    the expected behaviour as connection close after Alert, without use
    of ``next_sibling``.
...
To specify that script should expect connection close with or without
...
To actually connect to server and exchange messages, the runner needs to
...
by the server, etc.) runner raises an exception.
