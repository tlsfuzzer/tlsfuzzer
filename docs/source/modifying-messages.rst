...
To create malformed messages tlsfuzzer supports modifying them before sending.
...
connections.
It provides two nodes to achieve it: one to send message unencrypted and
one to send it using current connection status.
...
side (``content_type`` and ``data``) as well as one
...
You can find example use of it in
...
end of payload; ``size`` parameter specifies how many bytes and
``pad_byte`` specifies the value of the added bytes.

In other calling convention, it accepts literal bytes to add to payload using
the ``pad`` keyword argument.

For example, to add 10 bytes with value 0 at the end of ClientHello, write:
...
specify negative ``size`` or use the
:py:func:`~tlsfuzzer.messages.truncate_handshake`.

.. note::

    Sender can format ClientHello in two ways: with and without extensions.
    ClientHello with empty list of extensions differs from one without
...
For example, to change type of a ClientHello message to that of ServerHello
...
Each of them behaves differently on record layer level, thus modifying the
intermediate ciphertext requires use of different functions.

Fuzzing the MAC
---------------

Depending on cipher suite and negotiated extensions, to change the
authentication tag you need to use different functions.
...
:py:func:`~tlsfuzzer.messages.fuzz_message`: pass message to change as the
first argument and use the other two to specify bytes to either xor or
substitute.

For example, to invert the first and last bit of :term:`HMAC` in a record with
a Finished message use the following code:
...
For example, to invert the first and last bit of authentication tag in
a record with a Finished message in an :term:`AES-GCM` connection, use the
following code:
...
You can find example use of this method in
`test-chacha20.py
<https://github.com/tomato42/tlsfuzzer/blob/master/scripts/test-chacha20.py>`_
script.

On the extreme end, tlsfuzzer supports also changing the whole plaintext
...
While you can use :py:func:`~tlsfuzzer.messages.fuzz_plaintext` function
...
As single byte encodes the length of the padding, 255 bytes is the max length
of it (256 bytes including the byte encoding length).

The :term:`TLS` 1.3 defines padding differently, it combines it with
...
You can find example use of this node in
...
The :term:`TLS` protocol specifies max length of payload at 2\ :sup:`14` bytes.
...
With this limit unmodified, record layer fragments a 16385 byte message into
two records.

For example, to send ApplicationData record 1 byte larger than the
...
For example, to send a ClientHello in two records, first 2 bytes long,
use the following code:
...
You can find example use of those objects in
`test-large-hello.py
