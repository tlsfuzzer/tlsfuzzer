...
nonce size. If some nonce size has large (typically >0.01) p-value or
Wilcoxon value (found in respecive files) and the confidence intervals of the
bootstrap test is small (typically <1ns) for the specific nonce size, then
we can confirm that no side-channel is present for this nonce size eitherwise
if we have very small p-value or Wilcoxon value (typically <0.00001) then a
side-channel is present on for this nonce size.
