...
simple distribution, in many cases they are multi-modal and not normal.
...
Generally, script assumes that the values below 0.05 mean that the values
...
<https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test>`_ to verify
...
Generally sample size must be proportional to 1/e to detect effect of size e.
...
for the difference between a given pair of samples:
...
2. Use methods mentioned above to create isolated cores, watch out for
   hyperthreading
3. For RSA tests use small key, for CBC tests use fast cipher and hash.
...
Any test that collects timing information needs to provide the following
...
``----cpu-list`` No         Core IDs to use for running tcpdump
================ ========== ============================================
...
script with an output folder where ``timing.csv`` generated by extraction is
present.
