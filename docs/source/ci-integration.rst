...
You should take into consideration following aspects of server configuration:

..
    please do not renumber the following list, there are external references
    to it

1. How many certificates the server has set up and what types
...
13. Storing the private key in an :term:`HSM` or a smart card
...
.. warning::
    ...
    tests. Before any such modifcations you should have good understanding of
...
mark it as "expected failure" by specifying its name as a parameter to ``-x``
option.
