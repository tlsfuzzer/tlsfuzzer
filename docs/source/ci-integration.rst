...
While you can write one-off test cases using tlsfuzzer to test specific
issue, the development primarily focuses on use of it in a :term:`CI`
environment.
...
:term:`ECDSA` certificate available in the server.

To verify all features of an implementation you need to enable all features
in a server. In case features conflict with each-other, you need to test
them separately by running the relevant tests with every configuration.

You need to decide what kinds of configurations you want to test.
You should read and apply
`combinatorial testing
<https://www.nist.gov/programs-projects/automated-combinatorial-testing-software-acts>`_,
related to
`pairwise testing <https://en.wikipedia.org/wiki/All-pairs_testing>`_,
to select server configurations.

Configuration variables
-----------------------

Things that you should consider as having impact on server behaviour:

..
    please do not renumber the following list, there are external references
    to it

1. How many certificates the server has set up and what kinds

   * especially relevant for differences between ``rsaEncryption`` and
     ``rsassa-pss`` in Subject Public Key Info
2. :term:`SNI` configured or not
3. Testing the default host vs :term:`SNI` host of a :term:`SNI`\ -enabled
   server
4. Security level enabled in the library
...
5. No client certificates, requesting client certificates, or requiring client
   certificates
6. :term:`ALPN` (or :term:`NPN`) configured or not
...
8. Support for 0-RTT enabled or not
...
13. Private key stored in an :term:`HSM` or smart card

    * different features supported in the smart card—things like RSA-PSS
      or SHA-384
14. Specific extension or feature of the protocol disabled, enabled, or
    required
...
18. Interactions between implementation versions and session resumptions—
    what happens when a client resumes session from old library with new
    server (or vice versa, to simulate server downgrade)
...
expect that executing them the first time will find a lot of issues.
As such, you should start with running them one by one, manually, inspecting
test results and checking if they pass.

Many tests verify behaviour not directly related to main feature under
test—indicated by the name and the summary printed at the end of execution—
in such cases they provide extra command line options to make them
...
.. warning::
    ...
    tests, before changing it you should have good understanding of
    :term:`TLS` and oracle attacks—you need to verify that similarly malformed
    messages result in the exact same alerts.
    Please note that some features (e.g. padding in :term:`CBC` mode ciphers)
    have more than one script that tests them, so you need to adjust runs of
...
mark it as "expected fail" by specifying its name as a parameter to ``-x``
option.
In the latter case, you can also specify a pattern to match the returned error
against with the ``-X`` option.
...
script.
If you want to execute all tests in a script specify ``-n 0``.

Automation scripts
==================

Tlsfuzzer ships with few scripts to make using it in :term:`CI` easier:
``verify-scripts-json.py``, ``scripts_retention.py``, and
``verify-multiple-jsons.py``.

The ``scripts_retention.py`` starts servers based on passed configuration file.
To ensure reliable execution it verifies that the server can accept connections
