...
def main():
    ...
    if (
        'brotli' in compression_algorithms_list
        ...
        compression_algorithms_list.remove('brotli')
    if (
        'zstd' in compression_algorithms_list
        ...
        compression_algorithms_list.remove('zstd')

    compression_algorithms = {}

    compression_algorithms = {
        alg_name: getattr(CertificateCompressionAlgorithm, alg_name)
        for alg_name in compression_algorithms_list
    }
    ...
    node = node.add_child(ExpectCertificateRequest(include_extensions=[
    ...
    for name, size in [
        ...
        node = node.add_child(ExpectFinished())
        node = node.add_child(CompressedCertificateGenerator(
            override_uncompressed_length=size))
    ...
    node = node.add_child(ExpectFinished())
    node = node.add_child(CompressedCertificateGenerator(
        override_not_advertized_algorithm=True))
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                        AlertDescription.bad_certificate))
    node = node.add_child(ExpectClose())
    ...
    node = node.add_child(CompressedCertificateGenerator(
        ...
        override_algorithm=CertificateCompressionAlgorithm.brotli
    ))
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                        AlertDescription.bad_certificate))
    node = node.add_child(ExpectClose())
    ...
    # Additional bytes in compressed message end, reflected in size
    compressed_msg = zlib.compress(bytearray(b'\x00\x00\x00'))
    compressed_msg += b'\x00\x00'  # 2 more bytes at the end
    ...
    node = node.add_child(CompressedCertificateGenerator(
        certs=None,
        algorithm=CertificateCompressionAlgorithm.zlib,
        override_uncompressed_length=3,
        override_compressed_certificate_message=compressed_msg,
    ))
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                      AlertDescription.decode_error))
    node.next_sibling = ExpectClose()
    conversations["Additional bytes"] = conversation

    # Additional bytes in compressed message end, not reflected in size
    compressed_msg = zlib.compress(bytearray(b'\x00\x00\x00'))
    ...
    node = node.add_child(fuzz_message(CompressedCertificateGenerator(
        ...
        override_uncompressed_length=3,
        override_compressed_certificate_message=compressed_msg,
    ), substitutions={- len(compressed_msg) - 1: orig_comp_msg_len}))
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                      AlertDescription.decode_error))
    node.next_sibling = ExpectClose()
    conversations["Additional bytes, unreflected in size"] = conversation
    ...
    for size in sizes:
        ...
        node = node.add_child(CompressedCertificateGenerator(
            ...
            algorithm=CertificateCompressionAlgorithm.zlib,
            override_uncompressed_length=2**24-1,
            override_compressed_certificate_message=bytearray(size),
        ))
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                        AlertDescription.bad_certificate))
        node.next_sibling = ExpectClose()
