...
def main():
    ...
    for cipher in [CipherSuite.TLS_AES_128_GCM_SHA256,
        ...
        node = node.add_child(ExpectAlert())
        node.next_sibling = ExpectClose()
        conversations["max size plaintext, with {0}".format(
        ...
        conversations["too big ClientHello msg, with {0} bytes of padding,"
                      "no encryption keys, with {1} cipher".format(
        ...
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                          AlertDescription.record_overflow))
        node.add_child(ExpectClose())
        conversations["too big Finished msg, with {0} bytes of record layer padding,"
                      " handshake encryption keys, with {1} cipher".format(
                      padding_size, CipherSuite.ietfNames[cipher])] = conversation
        ...
        node = node.add_child(ExpectFinished())
        padding_size = 2**15
        node = node.add_child(FinishedGenerator(pad_left=padding_size // 2,
                                                pad_right=padding_size // 2))
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                          AlertDescription.record_overflow))
        node.add_child(ExpectClose())
        conversations["too big Finished msg, with {0} bytes of left and right padding,"
                      " handshake encryption keys, with {1} cipher".format(
                      padding_size // 2, CipherSuite.ietfNames[cipher])] = conversation
        ...
        conversations["too big plaintext, data size: 2**14 + 1, with {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
        ...
        node = node.add_child(FinishedGenerator())
        data = bytearray(b"GET / HTTP/1.0\r\n" +
                         b"X-test: " + b"A" * (2**14 - 28 + 256) +
                         b"\r\n\r\n")
        assert len(data) == 2**14 + 256
        node = node.add_child(ApplicationDataGenerator(data))
        ...
        node.next_sibling = ExpectAlert(AlertLevel.fatal,
                                        AlertDescription.record_overflow)
        node.next_sibling.add_child(ExpectClose())
        conversations["too big plaintext, size: 2**14 + 256, cipher {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
        ...
        node = node.add_child(FinishedGenerator())
        data = bytearray(b"GET / HTTP/1.0\r\n" +
                         b"X-test: " + getRandomBytes(2**14 - 28 + 256) +
                         b"\r\n\r\n")
        assert len(data) == 2**14 + 256
        node = node.add_child(ApplicationDataGenerator(data))
        ...
        node.next_sibling = ExpectAlert(AlertLevel.fatal,
                                        AlertDescription.record_overflow)
        node.next_sibling.add_child(ExpectClose())
        conversations["too big random plaintext, size: 2**14 + 256, cipher {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
        ...
        for val in [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80]:
            for pos in range(-1, -17, -1):
                ...
                node.next_sibling = ExpectAlert(AlertLevel.fatal,
                                                AlertDescription.record_overflow)
                node.next_sibling.add_child(ExpectClose())
                conversations_long["too big plaintext with size 2**14+256 and"
                                   "fuzzed tag with {0} on pos {1}, cipher {2}".format(
                    val, pos, CipherSuite.ietfNames[cipher])] = conversation
