...
def main():
    ...
    for cipher in [CipherSuite.TLS_AES_128_GCM_SHA256,
        ...
        conversations["too big plaintext, data size: 2**14 + 1, none ecryption, with {0}".format(
        ...
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                          AlertDescription.record_overflow))
        node.add_child(ExpectClose())
        conversations["too big ciphertext, size: 2**14 + 256, cipher {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
        ...
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                          AlertDescription.record_overflow))
        node.add_child(ExpectClose())
        conversations["too big random ciphertext, size: 2**14 + 256, cipher {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
        ...
        for val in [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80]:
            for pos in range(-1, -17, -1):
                ...
                node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                                  AlertDescription.record_overflow))
                node.add_child(ExpectClose())
                conversations["fuzz tag with {0} on pos {1}, cipher {2}".format(
        ...
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                          AlertDescription.record_overflow))
        node.add_child(ExpectClose())
        conversations["too big plaintext, size: 2**14 + 2, cipher {0}".format(
            CipherSuite.ietfNames[cipher])] = conversation
