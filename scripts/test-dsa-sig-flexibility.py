...
def main():
    ...
    if ciphers:
        if ciphers[0] not in CipherSuite.dheDsaSuites:
            raise ValueError("Ciphersuite not supported or does not use DHE_DSS key exchange")
    else:
        ciphers = [CipherSuite.TLS_DHE_DSS_WITH_AES_128_CBC_SHA]
    ...
    for h_alg in ("sha512", "sha384", "sha256", "sha224", "sha1"):
        ...
        ext[ExtensionType.supported_groups] = SupportedGroupsExtension()\
            .create(groups)
        sig_alg = [(getattr(HashAlgorithm, h_alg), SignatureAlgorithm.dsa)]
        ext[ExtensionType.signature_algorithms] = \
            SignatureAlgorithmsExtension().create(sig_alg)
        ext[ExtensionType.signature_algorithms_cert] = \
            SignatureAlgorithmsCertExtension().create(SIG_ALL)
        node = node.add_child(ClientHelloGenerator(
            ciphers + [CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV],
        ...
        node = node.add_child(ExpectAlert())
        node.next_sibling = ExpectClose()
        conversations["{0}+DSA only".format(h_alg)] = conversation
    ...
    ext[ExtensionType.supported_groups] = SupportedGroupsExtension()\
        .create(groups)
    sig_alg = [(HashAlgorithm.md5, SignatureAlgorithm.dsa)]
    ext[ExtensionType.signature_algorithms] = \
        SignatureAlgorithmsExtension().create(sig_alg)
    ext[ExtensionType.signature_algorithms_cert] = \
        SignatureAlgorithmsCertExtension().create(SIG_ALL)
    node = node.add_child(ClientHelloGenerator(
        ciphers + [CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV],
        extensions=ext))
    node = node.add_child(ExpectAlert(AlertLevel.fatal, AlertDescription.handshake_failure))
    node.add_child(ExpectClose())
    conversations["MD5+DSA only"] = conversation
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["SHA1+DSA implicit".format(h_alg)] = conversation
