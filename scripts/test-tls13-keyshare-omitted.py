...
def main():
    ...
    ext = {}
    groups = [GroupName.secp256r1]
    key_shares = [KeyShareEntry().create(groups[0], bytearray(b'\xab'*32))]
    ext[ExtensionType.key_share] = ClientKeyShareExtension().create(key_shares)
    ext[ExtensionType.supported_versions] = SupportedVersionsExtension()\
        .create([TLS_1_3_DRAFT, (3, 3)])
    ext[ExtensionType.supported_groups] = SupportedGroupsExtension()\
    ...
    ext[ExtensionType.signature_algorithms_cert] = SignatureAlgorithmsCertExtension()\
        .create(RSA_SIG_ALL)
    node = node.add_child(ClientHelloGenerator(ciphers, extensions=ext))
    node = node.add_child(ExpectAlert(description=AlertDescription.illegal_parameter))
    node = node.add_child(ExpectClose())
    conversations["invalid key_share with valid psk_key exhange modes"] = conversation
    ...
    print("Test with omitted or empty key_share extension.")
    print("Check if communication is rejected with correct alert message")
    print("with the TLS 1.3 server.\n")
    print("version: {0}\n".format(version))
