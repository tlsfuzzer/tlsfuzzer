...
def help_msg():
    ...
    print("                on specified interface")
    print(" -r repetitions How much timing samples should be gathered for each test")
    print("                100 by default")
    print(" -o directory   Specifies output directory for timing information")
...
def main():
    ...
    interface = None
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:t:n:a:l:o:i:r:", ["help",
                                                        "no-safe-renego",
                                                        "no-sni"])
    for opt, arg in opts:
        if opt == '-h':
    ...
    if timing and not interface:
        raise ValueError("In order to run timing tests, an interface has to be provided")
    ...
    if timing:
        timing_dir = TimingRunner.create_output_directory(outdir, sys.argv[0])
        log = Log(os.path.join(timing_dir, "class.log"))
    ...
    for c_name, c_test in ordered_tests:
        ...
        if timing and c_name != "sanity":
            log.add_class_name(c_name)
    ...
    if bad > 0:
        ...
        log.write_log()
        timing_runner = TimingRunner(log, timing_dir, host, port, interface)
        print("Running timing tests...")
        timing_runner.run(sampled_tests, run_only, run_exclude, repetitions)
