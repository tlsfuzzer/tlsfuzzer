...
def main():
    ...
    for i in range(1, 0x100):
        ...
        node = node.add_child(TCPBufferingDisable())
        node = node.add_child(TCPBufferingFlush())
        # Parsing certificates and messages can be done in parallel, so
        # a malformed ASN.1 may be detected before inconsistent lenghts
        # in Certificate.certificates field
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                         (AlertDescription.decode_error,
    ...
    for i in range(1, 0x100):
        ...
        node = node.add_child(TCPBufferingEnable())
        empty_cert_1 = X509()
        empty_cert_1.bytes = bytearray(b'\x00' * 3)
        empty_cert_2 = X509()
        empty_cert_2.bytes = bytearray(b'\x00' * 3)
        msg = CertificateGenerator(X509CertChain([cert, empty_cert_1, empty_cert_2]))
        node = node.add_child(msg)
        node = node.add_child(fuzz_message(msg, xors={-10: i}))
        node = node.add_child(ClientKeyExchangeGenerator())
        node = node.add_child(ChangeCipherSpecGenerator())
        node = node.add_child(FinishedGenerator())
        node = node.add_child(TCPBufferingDisable())
