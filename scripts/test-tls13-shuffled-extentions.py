...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:n:i:", ["help", "cookie"])
    for opt, arg in opts:
        if opt == '-h':
            host = arg
        elif opt == '-p':
    ...
    unassinged_ext_id = list(range(52, 65279))
    ...
    for ext_chunk in [unassinged_ext_id[j:j + chunk_size]
        ...
        ext[ExtensionType.signature_algorithms_cert] = SignatureAlgorithmsCertExtension()\
            .create(RSA_SIG_ALL)
        for ext_id in reversed(ext_chunk):
            ext[ext_id] = AutoEmptyExtension()
        node = node.add_child(ClientHelloGenerator(ciphers, extensions=ext))
        node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                      AlertDescription.illegal_parameter))
    ...
    print("Check correct handling for HelloRetryRequest message.")
