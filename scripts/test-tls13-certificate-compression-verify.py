...
def help_msg():
    ...
    print(" -E ext_spec    List of the extensions expected in server's CertificateRequest")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:x:X:n:k:c:a:E:", ["help"])
    for opt, arg in opts:
        if opt == '-h':
            hostname = arg
        elif opt == '-p':
    ...
    cr_expected_ext = {
            ExtensionType.compress_certificate:
                    gen_cln_ext_handler_compress_certificate(
                        algorithms=compression_algorithms_list
                    )
    }
    ...
    for alg_name, algorithm in compression_algorithms.items():
        ...
        node = node.add_child(ExpectAlert())
        node.next_sibling = ExpectClose()
        conversations["smoke, {0} server certificate".format(alg_name)] =\
                conversation
