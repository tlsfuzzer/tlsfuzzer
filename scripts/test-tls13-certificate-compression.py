...
def help_msg():
    ...
    print("                \"zlib,brotli,zstd\" by default")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:x:X:n:a:", ["help"])
    for opt, arg in opts:
        if opt == '-h':
            host = arg
        elif opt == '-p':
    ...
    for alg_name, algorithm in compression_algorithms.items():
        ...
        ext[ExtensionType.signature_algorithms_cert] = \
                SignatureAlgorithmsCertExtension().create(SIG_ALL)
        compression_algorithms = [algorithm]
        ext[ExtensionType.compress_certificate] = \
                CompressCertificateExtension().create(compression_algorithms)
        node = node.add_child(ClientHelloGenerator(ciphers, extensions=ext))
        node = node.add_child(ExpectServerHello())
        node = node.add_child(ExpectChangeCipherSpec())
        node = node.add_child(ExpectEncryptedExtensions())
    ...
    for unreasonable_compression_algorithm in UNREASONABLE_ALGORITHMS:
        ...
        ext[ExtensionType.signature_algorithms_cert] = \
                SignatureAlgorithmsCertExtension().create(SIG_ALL)
        compression_algorithms = [unreasonable_compression_algorithm]
        ext[ExtensionType.compress_certificate] = \
                CompressCertificateExtension().create(compression_algorithms)
        node = node.add_child(ClientHelloGenerator(ciphers, extensions=ext))
        node = node.add_child(ExpectServerHello())
