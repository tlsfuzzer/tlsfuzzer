...
def build_ext_aes128_ccm_8(dhe):
...
def build_ext_aes128_ccm(dhe):
    """ Build the AES ext with CCM and ciphers """
    ext = {}
    if dhe:
        groups = [GroupName.secp256r1,
                  GroupName.ffdhe2048]
...
def build_conn_graph(host, port, ccm_type_8, dhe, application_data_length):
    ...
    if ccm_type_8:
        (ext, ciphers) = build_ext_aes128_ccm_8(dhe)
    else:
        (ext, ciphers) = build_ext_aes128_ccm(dhe)
    ...
    data = bytearray(b"GET / HTTP/1.0\r\n" +
                     b"X-test: " + b"A" * (application_data_length) +
                     b"\r\n\r\n")
    assert len(data) == (application_data_length + 28)
...
def main():
    ...
    (conversation, node) = build_conn_graph(host, port, ccm_type_8=False,
                                            dhe=dhe,
                                            application_data_length=(2**14 - 28))
    ...
    (conversation, node) = build_conn_graph(host, port, ccm_type_8=False,
                                            dhe=dhe,
                                            application_data_length=(2**14 - 28 + 1))
    ...
    (conversation, node) = build_conn_graph(host, port, ccm_type_8=False,
                                            dhe=dhe,
                                            application_data_length=(2**14 + 1024 - 28))
    ...
    (conversation, node) = build_conn_graph(host, port, ccm_type_8=False,
                                            dhe=dhe,
                                            application_data_length=(2**14 + 1024 - 28 + 1))
