...
def main():
    ...
    cipherlist = [CipherSuite.TLS_RSA_WITH_AES_128_CCM,
                  CipherSuite.TLS_RSA_WITH_AES_256_CCM_8]
    if dhe:
        cipherlist = [CipherSuite.TLS_DHE_RSA_WITH_AES_128_CCM,
                      CipherSuite.TLS_DHE_RSA_WITH_AES_256_CCM_8]
    for cipher in cipherlist:
        ...
        n = 17
        if cipher == CipherSuite.TLS_RSA_WITH_AES_256_CCM_8 or \
                cipher == CipherSuite.TLS_DHE_RSA_WITH_AES_256_CCM_8:
            n = 9
        for val in [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80]:
            for pos in range(-1, -n, -1): 
                ...
                node.add_child(ExpectClose())
                conversations["fuzz tag with {0} on pos {1} - using {2}".format(val, pos, CipherSuite.ietfNames[cipher])] \
                        = conversation
