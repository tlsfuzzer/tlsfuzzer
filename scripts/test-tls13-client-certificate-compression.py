...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:x:X:n:C:k:c:", [
    ...
    for name, size in [
        ...
        size_bytes = int.to_bytes(size, 3, "big")
        conversation = Connect(host, port)
        node = conversation
        ext = {}
        groups = [GroupName.secp256r1]
        ...
        node = node.add_child(fuzz_message(CompressedCertificateGenerator(
            X509CertChain([cert])),
            substitutions=dict(enumerate(size_bytes, 6))))
    ...
    for algo in diff:
        ...
        node = node.add_child(ExpectClose())
        test_title = "non advertized algorithm - {0}".format(algo)
        conversations[test_title] = conversation

    # change the compression algorithm used to compress to a different
    # supported one
    for pair in combinations(compression_algorithms.items):
        conversation = Connect(host, port)
        node = conversation
        ext = {}
        groups = [GroupName.secp256r1]
    ...
    UNSUPPORTED_ALGORITHMS = {  # Should not lead to certificate compression
        0,                       # reserved
    ...
    for algo in UNSUPPORTED_ALGORITHMS:
        algo_bytes = int.to_bytes(algo, 2, "big")
        conversation = Connect(host, port)
        node = conversation
        ext = {}
        groups = [GroupName.secp256r1]
