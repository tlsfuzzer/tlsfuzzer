...
def help_msg():
    ...
    print("                may be specified multiple times")
    print(" -s             if present, test expect supported_groups extension")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:n:i:s", ["help"])
    for opt, arg in opts:
        if opt == '-h':
            host = arg
        elif opt == '-p':
    ...
    tls12_extensions = [ExtensionType.cert_type,  # 9
                        ExtensionType.ec_point_formats,  # 11
                        ExtensionType.srp,  # 12
                        ExtensionType.encrypt_then_mac,  # 22
                        ExtensionType.extended_master_secret,  # 23
                        ExtensionType.supports_npn,  # 13172
    ...
    unassinged_ext_id += list(range(22, 28))
    unassinged_ext_id += list(range(29, 41))
    ...
    for ext_chunk in [unassinged_ext_id[j:j + chunk_size]
        ...
        for ext_id in ext_chunk:
            ext[ext_id] = AutoEmptyExtension() if ext_id not in tls12_extensions\
                else TLSExtension(extType=ext_id).create(bytearray(0))
    ...
    for ext_chunk in [unassinged_ext_id[j:j + chunk_size]
        ...
        for ext_id in ext_chunk:
            ext[ext_id] = TLSExtension(extType=ext_id).create(next(random_payload).data)\
                if ext_id not in tls12_extensions else TLSExtension(extType=ext_id).create(bytearray(0))
        if ExtensionType.renegotiation_info in ext:
            ext[ExtensionType.renegotiation_info] = None
