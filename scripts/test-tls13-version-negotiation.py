...
def main():
	...
	node = node.add_child(ExpectAlert())
	node.next_sibling = ExpectClose()
	conversations["tls 1.3 negotiation with TLS 1." + str(version_id) + " in record layer"] = conversation
    ...
    ext[ExtensionType.signature_algorithms] = SignatureAlgorithmsExtension()\
        .create(sig_algs)
    node = node.add_child(ClientHelloGenerator(ciphers, extensions=ext))
    node = node.add_child(ExpectAlert(AlertLevel.fatal, 70))
    conversations["tls 1.10 only"] = conversation
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["fallback from TLS 1.10 to 1.2"] = conversation
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["fallback from TLS 1.10 to 1.1"] = conversation
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["TLS 1.10 in client hello legacy field"] = conversation
    ...
    node = node.add_child(ClientHelloGenerator(ciphers,
                                               ...
                                               ssl2=True))
    node = node.add_child(ExpectAlert())
    conversations["SSL 2.0 ClientHello with TLS 1.3 version and ciphersuites"] = conversation
