...
def main():
    ...
    # Supported groups
    group_names = GroupName.allFF + [GroupName.secp256r1, GroupName.secp384r1,
                                     GroupName.secp521r1, GroupName.x25519,
                                     GroupName.x448]

    # Unkown key_share, known supported_groups
    for group_name in group_names:
        for size in [2, 4, 8, 16, 32, 64, 128]:
            ...
            groups = [group_name, 0x1300]
            key_shares = [KeyShareEntry().create(0x1300, bytearray(b'\xab'*size))]
            ext[ExtensionType.key_share] = ClientKeyShareExtension().create(key_shares)
            ...
            conversations["unkown key_share with size {0}, group {1}".format(
                size, GroupName.toRepr(group_name))] = conversation

    # Unkown supported_groups and key_shares
    for size in [2, 4, 8, 16, 32, 64, 128]:
        conversation = Connect(host, port)
        node = conversation
        ciphers = [CipherSuite.TLS_AES_128_GCM_SHA256,
        ...
        conversations["unkown supported_groups and key_shares, size {0}".format(size)] = conversation

    # Know and unkown groups, unknown are listed first
    for group_name in group_names:
        conversation = Connect(host, port)
        node = conversation
        ciphers = [CipherSuite.TLS_AES_128_GCM_SHA256,
        ...
        conversations["Known and unkown groups, know group {0}".format(
            GroupName.toRepr(group_name))] = conversation
    ...
    print("Check that server replie with HRR, abort the connection")
    print("with handshake_failure or choose a known group from client list.")
