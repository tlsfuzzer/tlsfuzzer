...
def main():
    ...
    # Unknown key_shares, one known P-256 group and range of unkown groups in supported_groups
    for group_name, unknown_group in unknown_groups.items():
        for size in [64, 128, 256]:
            ...
            groups = [GroupName.secp256r1] + unknown_group
            ...
            conversations["only unknown key_share from {0} range, key_share of size {1} + P-256 in supported_groups".format(
                group_name, size)] = conversation

    # Unknown key_shares, one known FFDHE2048 group and range of unkown groups in supported_groups
    for group_name, unknown_group in unknown_groups.items():
        for size in [64, 128, 256]:
            conversation = Connect(host, port)
            node = conversation
            ...
            groups = [GroupName.ffdhe2048] + unknown_group
            key_shares = [KeyShareEntry().create(un_group, bytearray(b'\xab'*size)) for un_group in unknown_group]
            ...
            conversations["only unknown key_share from {0} range, key_share of size {1} + FFDHE2048 in supported_groups".format(
                group_name, size)] = conversation
    ...
    # Known P-256 group and unknown groups, unknown ones are listed first
    for group_name, unknown_group in unknown_groups.items():
        for size in [64, 128, 256]:
            conversation = Connect(host, port)
            node = conversation
            ...
            groups = unknown_group + [GroupName.secp256r1]
            key_shares = [KeyShareEntry().create(un_group, bytearray(b'\xab'*size)) for un_group in unknown_group]
            ...
            conversations["known group P-256 and unknown groups from {0} range, key_share of size {1}".format(
                group_name, size)] = conversation

    # Known FFDHE2048 group and unknown groups, unknown ones are listed first
    for group_name, unknown_group in unknown_groups.items():
        for size in [64, 128, 256]:
            conversation = Connect(host, port)
            node = conversation
            ...
            groups = unknown_group + [GroupName.ffdhe2048]
            key_shares = [KeyShareEntry().create(un_group, bytearray(b'\xab'*size)) for un_group in unknown_group]
            key_shares.append(key_share_gen(groups[-1]))
            ...
            conversations["known group FFDHE2048 and unknown groups from {0} range, key_share of size {1}".format(
                group_name, size)] = conversation
